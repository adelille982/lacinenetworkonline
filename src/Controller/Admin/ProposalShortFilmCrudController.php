...
class ProposalShortFilmCrudController extends AbstractCrudController
{
    ...
    public function configureActions(Actions $actions): Actions
    {
        $exportCsvAction = Action::new('exportCsv', 'Exporter en CSV', 'fa fa-download')
            ->linkToRoute('export_total_short_film_proposal_csv')
            ->createAsGlobalAction()
            ->addCssClass('btn btn-secondary')
            ->setIcon('fa fa-download');

        $markToFinance = Action::new('markToFinance', '🚀 À financer')
            ->linkToUrl(function (ShortFilm $film) {
                return $this->adminUrlGenerator
                    ->setController(self::class)
                    ->setAction('markAsToFinance')
        ...
        return $actions
            ->add(Crud::PAGE_INDEX, $markToFinance)
    ...
    {
        return $crud
            ->setEntityLabelInSingular('Court-métrage proposé')
            ->setEntityLabelInPlural('Tous les courts-métrages proposés')
    ...
    public function createIndexQueryBuilder(SearchDto $searchDto, EntityDto $entityDto, FieldCollection $fields, FilterCollection $filters): QueryBuilder
    {
        ...
        $qb->andWhere('entity.statutShortFilm IN (:validStatuts)')
            ->setParameter('validStatuts', ['Proposé']);
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        return [
                ...
                ->setFormTypeOptions([
                    ...
                    'query_builder' => function (\App\Repository\SpeakerRepository $repo) {
                        return $repo->createQueryBuilder('s')
                            ...
                            ->setParameter('types', ['Externe'])
                            ->setParameter('statut', 'Proposé');
                    },
                ])
                ->setHelp('Seuls les intervenants "Externe" et "Proposé" sont visibles.')
                ->setSortable(false)
            ...
            ImageField::new('imgProposal1', 'Image complémentaire 1')
                ->setBasePath('/images/courts-metrages/')
                ->setUploadDir('public/images/courts-metrages/')
                ->onlyOnIndex(),
            ...
            ImageField::new('imgProposal2', 'Image complémentaire 2')
                ->setBasePath('/images/courts-metrages/')
                ->setUploadDir('public/images/courts-metrages/')
                ->onlyOnIndex(),
            ...
            ImageField::new('imgProposal3', 'Image complémentaire 3')
                ->setBasePath('/images/courts-metrages/')
                ->setUploadDir('public/images/courts-metrages/')
                ->onlyOnIndex(),
            ...
            ImageField::new('imgProposal4', 'Image complémentaire 4')
                ->setBasePath('/images/courts-metrages/')
                ->setUploadDir('public/images/courts-metrages/')
                ->onlyOnIndex(),
            ...
            ImageField::new('imgProposal5', 'Image complémentaire 5')
                ->setBasePath('/images/courts-metrages/')
                ->setUploadDir('public/images/courts-metrages/')
                ->onlyOnIndex(),
            ...
            TextField::new('linkShortFilmProposal', 'Lien du court-métrage')
                ->onlyOnIndex()
                ->formatValue(function ($value, $entity) {
                    return $value ? sprintf('<a href="%s" target="_blank" class="btn btn-sm btn-primary">Visiter le lien</a>', $value) : '';
                })
                ->renderAsHtml(),

            FormField::addTab('Lien bande annnonce')->setIcon('fas fa-film'),
            TextField::new('linkTrailerShortFilmProposal', 'Lien bande-annonce')
                ->onlyOnIndex()
                ->formatValue(function ($value, $entity) {
                    return $value ? sprintf('<a href="%s" target="_blank" class="btn btn-sm btn-primary">Voir la bande-annonce</a>', $value) : '';
                })
                ->renderAsHtml(),

            BooleanField::new('formatDcp', 'Format DCP'),
    ...
    public function markAsToFinance(AdminContext $context, ManagerRegistry $doctrine): RedirectResponse
    {
        ...
        if ($shortFilm) {
            foreach ($shortFilm->getSpeakers() as $speaker) {
                $speaker->setTypeSpeaker('Stagiaire');
                $speaker->setStatutSpeaker('Validé');
            }

            $shortFilm->setStatutShortFilm('À financer');
            $em->flush();

            $this->addFlash('success', 'Court-métrage promu : À financer. Intervenant converti en Stagiaire validé.');
        } else {
            $this->addFlash('danger', 'Court-métrage introuvable.');
        }
    ...
    public function markAsProduced(AdminContext $context, ManagerRegistry $doctrine): RedirectResponse
    {
        ...
        if ($shortFilm) {
            ...
            $this->addFlash('success', 'Court-métrage validé : Produit. Intervenant converti en Réalisateur validé.');
        } else {
            $this->addFlash('danger', 'Court-métrage introuvable.');
        }
    ...
    #[Route('/export-total-short-film-proposal-csv', name: 'export_total_short_film_proposal_csv')]
    public function exportCsv(CsvExporterService $csvExporter, ManagerRegistry $doctrine): Response
    {
        ...
        return $csvExporter->export($data, $headers, 'export-tous-les-courts-metrages-proposee.csv');
    }
}
